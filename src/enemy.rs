...
pub fn update_enemy_movement(
    time: Res<Time>,
    mut query: Query<(&mut Enemy, &mut Transform, &mut TextureAtlasSprite)>,
) {
    let mut rng = rand::thread_rng();
    for (mut enemy, mut transform, mut sprite) in &mut query {

        match (enemy.block_timer, enemy.punch_timer, enemy.wait_timer) {
            (block_timer, punch_timer, wait_timer) if block_timer <= 0. && punch_timer <= 0. && wait_timer <= 0. => {
            ...
            (block_timer, punch_timer, _) if block_timer > 0. && punch_timer <= 0. => {
            ...
            (_, _, wait_timer) if wait_timer > 0. => {
